...
### Input
InputThe first line contains three integers n, k, x (1≤n≤200000, 0≤k≤1018, 1≤x≤1018) — the initial number of students, the number of students you can additionally invite, and the maximum allowed level difference.The second line contains n integers a1,a2,…,an (1≤ai≤1018) — the students levels.

### Output
OutputIn the only line print a single integer: the minimum number of stable groups you can split the students into.
NoteIn the first example you can invite two students with levels 2 and 11. Then you can split the students into two stable groups:   [1,1,2,5,8,11,12,13],  [20,22]. In the second example you are not allowed to invite new students, so you need 3 groups:   [1,1,5,5,20,20]  [60,70,70,70,80,90]  [420]
