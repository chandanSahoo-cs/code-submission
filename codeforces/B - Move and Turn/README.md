...
### Input
InputThe only line contains a single integer n (1≤n≤1000) — the number of steps the robot makes.

### Output
OutputPrint a single integer — the number of different possible locations after exactly n steps.
NoteIn the first sample case, the robot will end up 1 meter north, south, west, or east depending on its initial direction.In the second sample case, the robot will always end up √2 meters north-west, north-east, south-west, or south-east.
