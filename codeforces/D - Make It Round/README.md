...
### Input
InputThe first line contains a single integer t (1≤t≤104) —the number of test cases in the test.Each test case consists of one line.This line contains two integers: n and m (1≤n,m≤109). Where n is the old price of the good, and the number m means that you can increase the price n no more than m times.

### Output
OutputFor each test case, output on a separate line the roundest integer of the form n⋅k (1≤k≤m, k — an integer).If there are several possible variants, output the one in which the new price (value n⋅k) is maximal.If it is impossible to get a more rounded price, output n⋅m (that is, the maximum possible price).
