...
### Input
InputThe input consists of multiple test cases. The first line contains a single integer t (1≤t≤104) — the number of test cases. Description of the test cases follows.The first line of each test case contains two integers n and k (2≤n≤2⋅105, 1≤k≤109), the number of integers in the list, and the target value, respectively.The second line of each test case contains the n integers of the list a1,a2,…,an (−109≤ai≤109).It is guaranteed that the sum of n over all test cases is not greater that 2⋅105.

### Output
OutputFor each test case, print YES if you can achieve k with a sequence of n−1 operations. Otherwise, print NO.You may print each letter in any case (for example, "YES", "Yes", "yes", "yEs" will all be recognized as a positive answer).
